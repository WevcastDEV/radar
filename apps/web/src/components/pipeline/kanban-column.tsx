...
export function KanbanColumn({ 
  ...
  return (
    ...
    >
      {/* Column Header */}
      <div className="p-3.5 border-b border-border bg-accent/20 rounded-t-2xl">
        <div className="flex items-center justify-between mb-1.5">
          ...
          <span className="bg-background text-[11px] px-2 py-0.5 rounded-full font-bold border border-border text-foreground">
            {stage.leads.length}
          </span>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-2.5 space-y-2.5">
        ...
        {stage.leads.length === 0 && (
          <div className="h-28 flex flex-col items-center justify-center border-2 border-dashed border-border/80 rounded-xl text-xs text-muted-foreground gap-1 p-4 text-center">
            ...
            <span className="text-[10px] text-muted-foreground/70">Arraste ou use os botões para mover</span>
          </div>
        )}
      </div>
    </div>
